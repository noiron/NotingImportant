...
class Menu(object):
    ...
    def main(self, screen):
        ...
        while self.running:
            ...
            screen.blit(highscoresurf, highscoresurf.get_rect(
                            right = WIDTH - 50, bottom = HEIGHT - 50))
            menu.draw(screen)
            pygame.display.flip()

    def construct_highscoresurf(self):
        font = pygame.font.Font(None, 50)
        hightscore = load_score()
        text = "Highscore: {}".format(hightscore)
...
def construct_nightmare(size):
    ...
    for x in xrange(0, len(arr), boxsize):
        for y in xrange(0, len(arr[x]), boxsize):
            ...
            color = int(''.join([random.choice(vals) + random.choice(vals) for 
                            _ in range(3)]), 16)
